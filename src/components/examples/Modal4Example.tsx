        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn">
          ...
          <div className="relative bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-6 border border-slate-700 animate-slideUp">
            <div className="flex flex-col items-center text-center">
              ...
              <button
                onClick={() => setIsOpen(false)}
                className={\`w-full px-4 py-2 \${config.button} text-white rounded-lg transition-colors\`}
              >
                Dismiss
              </button>
            </div>
...
export default function Modal4Example() {
  ...
  const alertConfig = {
    success: { bg: 'bg-emerald-500/20', text: 'text-emerald-500', button: 'bg-emerald-600', title: 'Success!', message: 'Action completed successfully.' },
    error: { bg: 'bg-red-500/20', text: 'text-red-500', button: 'bg-red-600', title: 'Error!', message: 'Something went wrong.' },
    warning: { bg: 'bg-amber-500/20', text: 'text-amber-500', button: 'bg-amber-600', title: 'Warning!', message: 'Please review your action.' }
  };
  ...
  return (
    <div className="flex gap-3 p-6">
      <button onClick={() => openAlert('success')} className="px-6 py-3 bg-emerald-600 text-white rounded-lg">Success</button>
      <button onClick={() => openAlert('error')} className="px-6 py-3 bg-red-600 text-white rounded-lg">Error</button>
      <button onClick={() => openAlert('warning')} className="px-6 py-3 bg-amber-600 text-white rounded-lg">Warning</button>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-800 rounded-2xl max-w-md w-full p-6 border border-slate-700">
            <div className="flex flex-col items-center text-center">
              <div className={\\\`w-16 h-16 \\\${config.bg} rounded-full flex items-center justify-center mb-4\\\`}>
                <div className={config.text}>{config.title}</div>
              </div>
              <h3 className="text-xl font-bold text-slate-100 mb-2">{config.title}</h3>
              <p className="text-slate-400 mb-6">{config.message}</p>
